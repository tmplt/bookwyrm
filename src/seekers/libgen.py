...
class SoupError(Exception):
    def __init__(self, soup, error):
        self.soup = soup
        super().__init__('failed to parse soup: ' + error)
        ...
        for query in self.queries:
            for domain in DOMAINS:
                ...
                try:
                    ...
                    # save html in some temp file here for bug submission
                    self.log(Loglevel.error, 'unable to parse "%s"; somewhere a None appeared. Continuing anyway...' % f.url)
                    continue
