...
namespace bookwyrm::tui::screen {
    ...
    footer::footer() : base(curses::get_height() - 2, 0, 0, 0) {}

    void footer::paint()
    {
        /* Print the number of running plugins and found items. */
        if (plugin_count_ == 0) {
            print(0, 0, fmt::format("Search finished: I found {} items.", item_count_));
        } else {
            print(0,
                  0,
                  fmt::format(
        ...
        if (scroll_perc_ > -1) {
            print_right_align(0, fmt::format("({})", scroll_perc_));
        }
        ...
        if (worst_unread_.has_value()) {
            print_right_align(
                1, " You have unread logs! ", to_colour(worst_unread_.value()) | attribute::reverse | attribute::bold);
        }
    }
