...
namespace std {
    ...
    template <class T> inline void hash_combine(std::size_t &seed, const std::vector<T> &vec)
    {
        std::hash<T> hasher;
        for (auto &v : vec) {
    ...
    template <> struct hash<bookwyrm::core::exacts_t> {
    ...
    template <> struct hash<bookwyrm::core::nonexacts_t> {
    ...
    template <> struct hash<bookwyrm::core::misc_t> {
    ...
    template <> struct hash<bookwyrm::core::item> {
