...
namespace bookwyrm::core {
    ...
    namespace func {
        ...
        /*
         * Take two containers, zip over them and return a vector with the length of the
         * shortest passed container.
         *
         * NOTE: This is some ugly template programming. Please fix it.
         */
        template <typename Cont1, typename Cont2>
        auto zip(const Cont1 &ac, const Cont2 &bc)
            -> std::vector<std::pair<typename Cont1::value_type, typename Cont2::value_type>>
        {
        ...
        template <typename Set> inline bool any_intersection(const Set &a, const Set &b)
        {
    ...
    struct exacts_t {
        ...
        explicit exacts_t(std::pair<year_mod, int> yearmod, int volume, int number, const string &extension)
            : ymod(std::get<0>(yearmod)), year(std::get<1>(yearmod)), volume(volume), number(number), pages(empty),
              size(empty), extension(extension)
        {
        }

        explicit exacts_t(const py::dict &dict);

        exacts_t() : ymod(year_mod::unused), year(empty), volume(empty), number(empty), pages(empty), size(empty) {}

        bool operator==(const exacts_t &other) const;

        const year_mod ymod;
        const int year, volume, /* no associated flag */
        ...
        const std::array<int, 5> store = {{volume, number, pages}};
    };

    struct nonexacts_t {
        /* Holds strings, which are matched fuzzily. */

        explicit nonexacts_t(const vector<string> &authors,
                             const string &title,
                             const string &series,
    ...
    struct item {
    public:
        explicit item(const nonexacts_t ne, const exacts_t e) : nonexacts(ne), exacts(e), misc(), index(items_idx++) {}

        explicit item(const py::dict &dict) : nonexacts(dict), exacts(dict), misc(dict), index(items_idx++) {}
        ...
        bool operator<(const item &other) const { return index < other.index; }
